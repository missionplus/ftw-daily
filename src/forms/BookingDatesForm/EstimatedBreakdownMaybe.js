...
const estimatedTotalPrice = (offer, lineItems) => {
  const numericTotalPrice = lineItems.reduce((sum, lineItem) => {
    const numericPrice = offer ? convertMoneyToNumber(offer) : convertMoneyToNumber(lineItem.lineTotal);
...
const estimatedTransaction = (offer, lineItems, userRole) => {
  const now = new Date();
  ...
  const payinTotal =  estimatedTotalPrice(offer, customerLineItems);
  const payoutTotal = estimatedTotalPrice(offer, providerLineItems);
...
const EstimatedBreakdownMaybe = props => {
  const { unitType, offer } = props.bookingData;
  ...
  const tx =
    ...
    lineItems
      ? estimatedTransaction(offer, lineItems, userRole)
      : null;
